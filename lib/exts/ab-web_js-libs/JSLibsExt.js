...
export default class JSLibsExt extends Ext {
    ...
    addLib(libName, libPath) {
        ...
        this.watch(libName, ['add', 'unlink', 'change'], [path.join(libPath, '**/*.js')]);
    }
    async #createScript_Async(scriptInfo) {
        ...
        let wb = new WebBuilder(scriptInfo.libName, libPath, path.join(this.#buildPath, scriptInfo.libName));
        wb.addParser((code, fileFSPath, buildFSPath) => {
            if (this.builder.isType('rel'))
                return code;
            let filename = path.relative(path.dirname(buildFSPath), fileFSPath).replace(/\\/g, '/');
            let script = null;
            try {
        ...
        try {
            let builtFSPath = await wb.buildScript_Async(scriptInfo.scriptPath, false);
            if (scriptInfo.eventType !== 'change')
                this.#js.addScript(`js-libs.${scriptInfo.libName}`, builtFSPath);
            let relScriptPath = path.relative(this.builder.settings.config.index, scriptInfo.scriptPath).replace(/\\/g, '/');
            this.console.log(`Built: '${scriptInfo.libName}:${relScriptPath}'.`);
        }
        catch (err) {
            this.console.error(`Error when building '${scriptInfo.libName}:` +
                `${scriptInfo.scriptPath}'.`, err.stack);
            return false;
        }
        return true;
    }
    ...
    __parse(config) {
        ...
        for (let libName in config.libs) {
            this.addLib(libName, path.join(buildSettings.initDir, config.libs[libName]));
        }
