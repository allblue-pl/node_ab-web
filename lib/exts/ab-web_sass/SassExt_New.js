...
export default class SassExt extends Ext {
    ...
    constructor(builder) {
        ...
        this.#sourceDirPath = buildConfig.index;
        this.#sourceDirPath_Rel = relDir;
    ...
    #getSources(fsPaths) {
        ...
        for (let fsPath of fsPaths.variables) {
            let relativePath = path.relative(this.#sourceDirPath_Rel, fsPath);
            relativePath = relativePath.replace(/\\/g, '/');
            sources['_default'] += '@use "ab-web:' + relativePath + '";\r\n';
            for (let variant of this.#variants) {
                sources[variant] += '@use "ab-web:' + relativePath + '";\r\n';
            }
        ...
        for (let substyle of this.#substyles) {
            ...
            for (let fsPath of fsPaths[`substyles.${substyle}.variables`]) {
                let relativePath = path.relative(this.#sourceDirPath_Rel, fsPath);
                relativePath = relativePath.replace(/\\/g, '/');
                sources[substyle] += '@use "ab-web:' + relativePath + '";\r\n';
                this.console.log('    - ' + relativePath);
            }
        }
        // sassSource += '\r\n';
    ...
    #parseSource(url) {
        ...
        if (!fs.existsSync(urlPath)) {
            this.console.error(`Imported file '${url}' does not exist.`);
            return '';
        }
        let data = fs.readFileSync(urlPath);
        let sass = data.toString();
        sass = sass.replace(/@use\s*([\'"])\s*/g, '@use $1' +
            path.dirname(url) + '/');
        let relativeSass = '';
    ...
    async __build() {
        ...
        for (let sourceAlias in this.#sources) {
            let sourcePath = sourceAlias === '_default' ?
                ...
                sourceAlias + '.css');
            promises.push((async () => {
                let result = await dartSass.compileStringAsync(this.#sources[sourceAlias], {
                    url: new URL(sourcePath),
                    loadPaths: [this.#sourceDirPath_Rel],
                    importers: [
                        {
                            canonicalize: (url, context) => {
                                console.log('Testing canonicalize', url, context);
                                return new URL(url);
                            },
                            load: (canonicalUrl) => {
                                return {
                                    contents: this.#parseSource(canonicalUrl.href),
                                    syntax: "scss",
                                };
                            },
                        }
        ...
        try {
            ...
            this.console.error("Error Compiling Sass: ", err.toString());
            return false;
        }
