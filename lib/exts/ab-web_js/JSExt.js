...
export default class JSExt extends Ext {
    ...
    addScriptsGroup(groupId, props = { values: [] }, type = "compile") {
    ...
    removeScript(groupId, scriptPath, type = "compile") {
    ...
    #sortScriptPaths(fsPaths, watchedFSPaths) {
        ...
        for (let fsPath of fsPaths) {
            ...
            for (let i = 0; i < watchedFSPaths.length; i++) {
                if (anymatch([watchedFSPaths[i].replaceAll("\\", "/")], fsPath.replaceAll("\\", "/"))) {
                    fsPaths_Helper.push({
                        order: i,
                        fsPath: fsPath,
                    });
    ...
    __build() {
        ...
        if (this.builder.isType('rel')) {
            ...
            try {
                ...
                let modulePaths_CoreJSBundle = path.join(this.builder.settings.config.dev, "node_modules", "core-js-bundle");
                // __dirname + '/../../node_modules/core-js-bundle/
                let modulePaths_RegeneratorRuntime = path.join(this.builder.settings.config.dev, "node_modules", "regenerator-runtime");
                // __dirname + '/../../node_modules/regenerator-runtime
                let js_Include_Code = js_Include_Result === null ?
                    '' : js_Include_Result.code;
                fs.writeFileSync(this.#scriptPath_Include, js_Include_Code + "\r\n" +
                ...
                // }, '');
                this.#header.addScriptUri_PostBody('js.min', this.uri(this.#scriptPath_Include));
                // this.#header.addTag_PostBody('js.min', 'script', {
                //     src: this.uri(this.#scriptPath_Min),
                //     // type: 'text/javascript',
                // }, '');
                this.#header.addScriptUri_PostBody('js.min', this.uri(this.#scriptPath_Min));
                this.#header.build();
            ...
            for (let type of types) {
                ...
                for (let [groupId, scriptPaths] of scriptGroups) {
                    this.console.info('    - ' + groupId);
                    scriptPaths = this.#sortScriptPaths(scriptPaths, this.#watchedFSPaths[type]);
                    for (let fsPath of scriptPaths) {
                        let relPath = null;
                        try {
