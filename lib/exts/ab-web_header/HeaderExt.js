...
export default class HeaderExt extends Ext {
    ...
    addTag_PostBody(groupId, tagName, tagAttribs) {
    ...
    addTag_Header(groupId, tagName, tagAttribs) {
    ...
    addScriptUrisGroup_Header(groupId, props = { values: [] }) {
        this.#header_ScriptUrisGroups.add(groupId, props);
    }
    addScriptUrisGroup_PostBody(groupId, props = { values: [] }) {
    ...
    getHtml_Header() {
        ...
        if (!this.builder.isType('rel')) {
            if (this.#exportHash.includes('js')) {
                let scriptRelPath = path.relative(buildSettings.config.index, this.#script_FilePath)
                    .replace(/\\/g, '/');
    ...
    __build() {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                fs.writeFileSync(this.#header_Scripts_FilePath, this.getHtml_Header_Scripts());
                let json_Header = {
                    scriptUris: this.#header_ScriptUrisGroups.getValues_AsArray(),
                };
                fs.writeFileSync(this.#header_Scripts_JSON, JSON.stringify(json_Header));
                fs.writeFileSync(this.#postBody_FilePath, this.getHtml_PostBody());
                fs.writeFileSync(this.#postBody_Scripts_FilePath, this.getHtml_PostBody_Scripts());
                let json_PostBody = {
                    scriptUris: this.#postBody_ScriptUrisGroups.getValues_AsArray(),
                };
                fs.writeFileSync(this.#postBody_Scripts_JSON, JSON.stringify(json_PostBody));
            }
            catch (err) {
                reject(err);
                return;
