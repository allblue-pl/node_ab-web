...
class abWeb_Class {
    ...
    _findExts()
    {
        ...
        let extPaths = fs.readdirSync(default_exts_path);
        for (let extPath of extPaths) {
            let ext_name = extPath.substring('ab-web_'.length);
            this._extClasses.set(ext_name, require(path.join(
                    default_exts_path, extPath)));
        }
    ...
    _parse(fsPath)
    {
        let config_path = path.resolve(fsPath);
        ...
        for (let [ extPath, ext ] of this._exts) {
            ...
            if (extPath in this._config._config)
                ext_config = this._config._config[extPath];
