...
class abWeb_Class {
    ...
    constructor() { let self = this;
        ...
        this._temp_Parsed = false;

        this._exec_Args = null;
    }

    config(devPath) {
        return new Config(this._buildInfo, this._config._configPath,
                devPath);
    }

    exec(config, buildType = 'dev') {
        js0.args(arguments, js0.RawObject, js0.Enum([ 'dev', 'rel' ]));
        js0.typeE(config, js0.Preset({
            ...
            args: [ js0.RawObject, js0.Default({}) ],
            init: js0.ArrayItems('string'),
            exts: js0.ArrayItems('string'),
        }));

        this._exec_Args = config.args;
    ...
    _createTask_Parse() {
        return new abTasks.Task('parse', (fsPaths, args) => {
    ...
    _parse(fsPath) {
        if (this._temp_Parsed)
            return;
        this._temp_Parsed = true;

        let configPath = path.resolve(fsPath);
        let configRequirePath = require.resolve(configPath);

        this._config.setConfigPath(path.dirname(configPath));
        ...
        let configInfo = require(configRequirePath);
        configInfo = typeof configInfo === 'function' ? 
                configInfo(this._exec_Args) : configInfo;

        this._config.parse(configInfo.getData());
        ...
        for (let [ extPath, ext ] of this._exts) {
            ...
            ext.__parse_Pre(extConfig, path.dirname(configPath));
        }
        ...
        for (let [ extPath, ext ] of this._exts) {
            ...
            ext.__parse(extConfig, path.dirname(configPath));
        }
    }
